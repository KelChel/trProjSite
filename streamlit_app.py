...
st.write("в выходные активность пользователей (вовлеченность) гораздо выше, чем в будни./nможет быть есть какая-то еще закономерность? может, рубрики особые? или еще что? анализируем два датафрейма (будни и выходные) чтобы узнать./nнадо поделать facet grids или построить разные графики, чтобы посмотреть, какие есть закономерности между несколькими значениями")
